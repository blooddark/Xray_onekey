...
function configure_nginx() {
  ...
  if [[ $compatible_nginx_conf == "yes" ]]; then
    wget -O ${domain}.conf https://raw.githubusercontent.com/blooddark/Xray_onekey/nginx_forward/config/web_compatible.conf
  elif [[ $compatible_nginx_conf == "no" ]]; then
    wget -O ${domain}.conf https://raw.githubusercontent.com/blooddark/Xray_onekey/nginx_forward/config/web.conf
  fi
  sed -i "s/xxx/${domain}/g" ${nginx_conf}
  modify_port
  modify_nginx_other
